...
havok <- function(xdat, dt = 1, stackmax = 100, lambda = 0,
  ...
  if (discrete == FALSE){
    ...
    res <- list(HAVOK, params, dx, r, sys, Theta, Xi, V, U, sigs, x)
    names(res) <- c("havokSS", "params", "dVrdt", "r", "sys", "normTheta", "Xi", "Vr", "Ur", "sigsr", "Vr_aligned")
    class(res) <- "havok"
    return(res)
    ...
    res <- list(HAVOK, params, dx, r, sys, Xi, U, sigs, V, x)
    names(res) <- c("havokSS", "params", "dVrdt", "r", "sys", "Xi", "Ur", "sigsr", "Vr","Vr_aligned")
    class(res) <- "havok"
    return(res)
  }
